...
/*Срок работы SQL Server после последнего перезапуска
Системная база данных tempdb пересоздаётся при каждом перезапуске SQL Server. Вот один из методов определения времени последнего перезапуска сервера.*/
...
FROM    sys.tables 
WHERE   type = 'U';
/*3.	Расположение файлов баз данных*/
EXEC sp_Helpfile; 
--OR 
SELECT  @@Servername AS Server ,
        DB_NAME() AS DB_Name ,
...
І ще один метод визначкння кількості таблиць*/
...
Select 'Hospital2.dbo.Posts' as DBName, count(*) as Count From dbo.Posts;
Select 'Hospital2.dbo.Sick_Leave' as DBName, count(*) as Count From dbo.Sick_Leave;
Select 'Hospital2.dbo.Travels_Leave' as DBName, count(*) as Count From dbo.Travels_Leave;
Select 'Hospital2.dbo.Units' as DBName, count(*) as Count From dbo.Units;
Select 'Hospital2.dbo.Workers' as DBName, count(*) as Count From dbo.Workers;
/*І ще один метод визначкння кількості таблиць*/
...
ORDER BY Tablename ,
        Rowcnt; 
DROP TABLE #rowcount;
/*5.	Поиск кластерных индексов и  куч (таблиц без кластерных индексов)*/
...
/*
Гарантированное получение полного сканирования таблицы при выполнении любого запроса
...
/*
Намного более продвинутая версия этого запроса представлена курсором, 
собирающим информацию по всем таблицам всех баз данных на сервере.
 Курсоры имеют невысокую производительность, 
 но перемещение по разным базам данных – это отличное применение для них
*/
...
--DROP TABLE tempdb..TempResults;
/*
В SQL Server, в некоторых случаях, мы можем обновлять данные с использованием представления.
 Чтобы получить представление «только для чтения», можно использовать SELECT DISTINCT при его создании. 
 Данные «через» представление можно менять только в том случае, 
 если каждой строке представления соответствует только одна строка в «базовой» таблице. 
 Любое представление, не отвечающее этому критерию, т.е. построенное на нескольких таблицах, 
 или с использованием группировок, агрегатных функций и вычислений, будет доступно только для чтения
...
EXEC sp_msdependencies N'Sales.Customer',null, 4 -- Change Table Name
